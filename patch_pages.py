...
(function(){
  ...
  function ewAdvice(cur, fib, lab){
    var c=cur||''; var stop=null, target=null, act='', tone='neutral';
    try{
    ...
    if(/5/.test(c)||/A浪调整/.test(c)){ act='末段/调整初：分批止盈、不追高；放量滞涨警惕失败浪'; tone='b-amber'; }
    else if(/4浪/.test(c)||/4/.test(c)){ act='第4浪回调是低吸区（大趋势向上时），轻仓、止损第4浪低点下破'; tone='b-green'; }
    else if(/3/.test(c)||/主升/.test(c)){ act='主升段：顺势持有，回踩加仓；止损第4浪低点下方'; tone='b-green'; }
    else if(/2浪/.test(c)||/2/.test(c)){ act='第2浪回调企稳可试多，止损第1浪起点下方'; tone='b-green'; }
    else if(/C浪末端/.test(c)||/C/.test(c)){ act='C浪末端临近布局区，不接飞刀，等放量企稳再介入'; tone='b-amber'; }
    else if(/B浪/.test(c)){ act='B浪反抽减仓，防C浪下杀'; tone='b-amber'; }
    else { act='结构不清晰，观望等待确认'; tone='neutral'; }
    if(fib&&fib.ctarget) target=fib.ctarget;
    return {act:act, stop:stop, target:target, tone:tone};
  }
  function analyzeElliott(klines, label){
    label=label||'标的';
    ...
    return {label:label, pct:pct, lab:lab, up:up, current:cur, fib:fib, sub:sub, advice:adv, conf:conf};
  }
  function drawElliott(res){
    ...
    if(a){
      ...
      box+='<div style="font-size:13px;color:var(--t1);font-weight:600">💡 操作参考</div>';
      box+='<div style="font-size:13px;color:var(--t1);margin-top:4px">'+esc(adv.act)+'</div>';
      ...
      box+='<div style="font-size:11px;color:var(--t4);margin-top:4px">波浪划分具概率性，须结合大趋势/估值/资金，非投资建议。</div>';
      box+='</div>';
      a.innerHTML=box;
    }
  }
  function loadElliott(){
    function run(tsCode, name){
      ...
      ewKline(tsCode,'day').then(function(kl){
        if(!kl||kl.length<60){ drawElliott({error:'日K线获取不足（'+(kl?kl.length:0)+'根），请稍后重试'}); return; }
        drawElliott(analyzeElliott(kl, name));
    ...
    if(sel&&btn){
      function ewName(){ return (sel.options && sel.selectedIndex!=null && sel.options[sel.selectedIndex]) ? sel.options[sel.selectedIndex].text : sel.value; }
      btn.onclick=function(){ run(sel.value, ewName()); };
      run(sel.value, ewName());
    }
    ...
    if(cbtn&&cinput){
      cbtn.onclick=function(){
        ...
        ewKline(getTencentCode(code),'day').then(function(kl){
          if(cmsg)cmsg.textContent='';
          if(!kl||kl.length<60){ drawElliott({error:esc(code)+' 日K线不足，无法分析'}); return; }
          var nm=code; try{ if(window.__EW_NAMES__&&window.__EW_NAMES__[code]) nm=window.__EW_NAMES__[code]; }catch(e){}
          drawElliott(analyzeElliott(kl, nm+' ('+code+')'));
  ...
  function boot(){ loadLiDaxiao(); loadSynthesis(); loadAuto(); loadBlue(); loadGuard(); loadTeam(); loadSent(); loadWatch(); loadElliott(); applyHash(); }
